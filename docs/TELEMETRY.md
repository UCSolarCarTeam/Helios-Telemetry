...
This is the tab named Race, and we use this tab for viewing the laps of the drivers who drove them. We store the drivers in PostgreSQL in the [driver table](./AMPLIFY.md#driver-table), and the laps in the [lap table](./AMPLIFY.md#lap-table). The drivers and the laps are associated by the shared [Rfid](./TELEMETRY.md#rfid) identifier.
