...
export class DatabaseService {
  ...
  constructor() {
    ...
    this.telemetryPacketRepo = this.dataSource.getRepository(TelemetryPacket);
  }
...
export class TelemetryService {
  ...
  /**
   * Insert complete telemetry packet into flattened table
   * All sensor data is stored in a single wide table for simplified insertions
   */
  async insertTelemetryPacket(packet: ITelemetryData): Promise<void> {
    const timestamp = new Date(packet.TimeStamp * 1000);
    const Rfid = packet.Pi.Rfid;

    // Flatten all nested data structures into a single object
    // Motor details are prefixed with Motor0_ and Motor1_
    const flattenedData: Partial<TelemetryPacket> = {
      // Metadata
      RaceName: packet.Title,
      Rfid,
      // Primary keys
    ...
    // Single insert into the flattened table
    await this.telemetryPacketRepo.save(flattenedData);
  }

  /**
   * Query telemetry data between dates from the flattened table
   */
  async getTelemetryBetweenDates(
    ...
    const queryBuilder = this.telemetryPacketRepo
    ...
    if (rfid) {
      queryBuilder.andWhere("t.Rfid = :rfid", { rfid });
    }
  ...
  /**
   * Get latest telemetry packet for a specific RFID
   */
  async getLatestTelemetry(rfid: string): Promise<TelemetryPacket | null> {
    return this.telemetryPacketRepo.findOne({
  ...
  /**
   * Batch insert multiple telemetry packets
   */
  async insertTelemetryPacketBatch(packets: ITelemetryData[]): Promise<void> {
    const flattenedPackets = packets.map((packet) => {
      const timestamp = new Date(packet.TimeStamp * 1000);
      const Rfid = packet.Pi.Rfid;
    ...
    await this.telemetryPacketRepo.save(flattenedPackets);
  }
}
