...
export class LapController implements LapControllerType {
  ...
  public startTimers() {
    this.timerInterval = setInterval(() => {
      this.raceInfo.timeLeft -= 1000000;
  ...
  public async handlePacket(packet: ITelemetryData) {
    ...
    if (!packet.B3.RaceMode) this.cleanUp();
    else if (this.timerInterval == null && packet.B3.RaceMode) this.startRace();
  ...
  private checkLap(packet: ITelemetryData) {
    const inProximity =
      getDistance(
        packet.Telemetry.GpsLatitude, // 51.081021
        packet.Telemetry.GpsLongitude, // -114.136084
        this.finishLineLocation.lat,
        this.finishLineLocation.long,
      ) <= 0.01; // 0.01 km = 10 m
