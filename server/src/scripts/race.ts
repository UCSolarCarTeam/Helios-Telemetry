...
export default class RaceService {
  ...
  getSecondsRemainingUntilChargedOrDepleted (averagePackCurrent: number, packAmpHours: number) {
    ...
    const hoursUntilChargedOrDepleted = amphoursLeft / Math.abs(averagePackCurrent)
  ...
  getDistanceTraveled (packetArray: TelemetryPacket[]) {
    ...
    const chronologicalArray = packetArray.slice(0).reverse()
    const motor0DistanceTravelledTotal = this.calculateMotorDistance(chronologicalArray, 0)
    const motor1DistanceTravelledTotal = this.calculateMotorDistance(chronologicalArray, 1)
  ...
  getAveragePowerIn (packetArray: TelemetryPacket[]) {
    ...
    const mpptPowerIn = packetArray.map((packet) => {
      let arrayPower = 0
    ...
    const regenPowerIn = packetArray.map((packet) => {
      let regen = 0
